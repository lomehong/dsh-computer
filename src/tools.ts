...
interface ComputerServiceLike {
  /** 执行一个 worker 操作，返回操作结果对象。 */
  run<T = Record<string, unknown>>(op: string, args?: Record<string, unknown>): Promise<T>
  /** 设置开关：电脑操作总开关（关闭时工具返回明确错误）。 */
  enabled(): boolean
}
...
function baseOutput(properties: Record<string, unknown>): JsonSchemaLike {
  return {
    type: 'object',
...
export function apply(ctx: Context): void {
  ...
  try {
    const service = (): ComputerServiceLike => {
      const svc = host.get?.('computer') as ComputerServiceLike | undefined
      if (svc === undefined || typeof svc.run !== 'function') {
        throw new Error('computer 服务不可用（插件未启用或 worker 未就绪）')
      }
      return svc
    }
    const wrap = (fn: (svc: ComputerServiceLike, args: unknown) => Promise<unknown>) =>
      async (args: unknown) => {
        const svc = service()
        // 停用检查由服务自身契约决定（而非 preset/闭包）：任何服务实现均可独立管控
        if (!svc.enabled()) throw new Error('电脑操作已在设置中停用（computer 设置节）')
        return fn(svc, args)
      }
    ...
    tools.register({
      ...
      output: {
        ...
        render: (_args, value) => {
          const v = value as { path?: string; width?: number; height?: number }
          return [{ type: 'text', text: `截图已保存：${v.path ?? ''}（${v.width ?? 0}×${v.height ?? 0}），请用 read_image 查看` }]
        },
      },
      execute: wrap(async (svc) => {
        const shot = await svc.run<{ path: string; width: number; height: number }>('screenshot')
    ...
    tools.register({
      name: 'computer_click',
      description:
        '在屏幕指定坐标处点击鼠标（坐标 = 截图像素坐标，原点为截图左上角）。' +
      ...
      output: {
        ...
        render: (_args, value) => {
          const v = value as { x?: number; y?: number; button?: string; double?: boolean; error?: string }
          if (v.error !== undefined) return [{ type: 'text', text: `点击失败：${v.error}` }]
      ...
      execute: wrap(async (svc, args) => {
        const a = (args ?? {}) as { x?: number; y?: number; button?: string; double?: boolean }
        if (typeof a.x !== 'number' || typeof a.y !== 'number') throw new Error('x 与 y 坐标必填（数字）')
    ...
    tools.register({
      ...
      output: {
        ...
        render: (_args, value) => {
          const v = value as { length?: number; error?: string }
          if (v.error !== undefined) return [{ type: 'text', text: `输入失败：${v.error}` }]
          return [{ type: 'text', text: `已输入 ${v.length ?? 0} 个字符` }]
        },
      },
      execute: wrap(async (svc, args) => {
        const a = (args ?? {}) as { text?: string }
        if (typeof a.text !== 'string' || a.text === '') throw new Error('text 必填（非空字符串）')
    ...
    tools.register({
      ...
      output: {
        ...
        render: (_args, value) => {
          const v = value as { combo?: string; error?: string }
          if (v.error !== undefined) return [{ type: 'text', text: `按键失败：${v.error}` }]
          return [{ type: 'text', text: `已按下 ${v.combo ?? ''}` }]
        },
      },
      execute: wrap(async (svc, args) => {
        const a = (args ?? {}) as { combo?: string }
        if (typeof a.combo !== 'string' || a.combo === '') throw new Error('combo 必填（如 win+r）')
    ...
    tools.register({
      ...
      output: {
        ...
        render: (_args, value) => {
          const v = value as { direction?: string; clicks?: number; error?: string }
          if (v.error !== undefined) return [{ type: 'text', text: `滚动失败：${v.error}` }]
          return [{ type: 'text', text: `已向 ${v.direction ?? 'down'} 滚动 ${v.clicks ?? 0} 格` }]
        },
      },
      execute: wrap(async (svc, args) => {
        const a = (args ?? {}) as { direction?: string; clicks?: number; x?: number; y?: number }
        const direction = a.direction === 'up' ? 'up' : 'down'
        const clicks = Math.max(1, Math.min(20, Math.round(a.clicks ?? 3)))
        const payload: Record<string, unknown> = { direction, clicks }
        if (typeof a.x === 'number' && typeof a.y === 'number') { payload.x = Math.round(a.x); payload.y = Math.round(a.y) }
    ...
    tools.register({
      ...
      description:
        '列出当前所有可见顶层窗口（z 序，前台在前）：标题、进程 id、位置尺寸。' +
      ...
      output: {
        ...
        render: (_args, value) => {
          const v = value as { count?: number; titles?: string; error?: string }
          if (v.error !== undefined) return [{ type: 'text', text: `窗口列举失败：${v.error}` }]
          return [{ type: 'text', text: `共 ${v.count ?? 0} 个可见窗口：\n${v.titles ?? ''}` }]
        },
      },
      execute: wrap(async (svc) => {
        const result = await svc.run<{ count: number; windows: Array<{ title: string; foreground: boolean }> }>('window-list')
    ...
    tools.register({
      ...
      output: {
        ...
        render: (_args, value) => {
          const v = value as { focused?: boolean; pid?: number; title?: string; error?: string }
          if (v.error !== undefined || v.focused !== true) return [{ type: 'text', text: `聚焦失败：${v.error ?? '未知原因'}` }]
          return [{ type: 'text', text: `已聚焦窗口「${v.title ?? ''}」（pid ${v.pid ?? 0}）` }]
        },
      },
      execute: wrap(async (svc, args) => {
        const a = (args ?? {}) as { match?: string }
        if (typeof a.match !== 'string' || a.match === '') throw new Error('match 必填（窗口标题子串）')
    ...
    tools.register({
      ...
      description:
        '读写系统剪贴板。action=get 读取当前文本；action=set 写入 text。' +
      ...
      output: {
        ...
        render: (_args, value) => {
          const v = value as { action?: string; length?: number; text?: string; error?: string }
          if (v.error !== undefined) return [{ type: 'text', text: `剪贴板操作失败：${v.error}` }]
          if (v.action === 'set') return [{ type: 'text', text: `已写入剪贴板（${v.length ?? 0} 字符）` }]
          const preview = (v.text ?? '').slice(0, 200)
          return [{ type: 'text', text: `剪贴板内容（${v.length ?? 0} 字符）：${preview}` }]
        },
      },
      execute: wrap(async (svc, args) => {
        const a = (args ?? {}) as { action?: string; text?: string }
        const action = a.action === 'set' ? 'set' : 'get'
        const payload: Record<string, unknown> = { action }
        if (action === 'set') {
          if (typeof a.text !== 'string') throw new Error('action=set 时 text 必填')
          payload.text = a.text
        }
        const result = await svc.run<{ action: string; length?: number; text?: string }>('clipboard', payload)
        return result
      }),
    })
  } catch (e) {
    try { console.warn('[dsh-computer] 工具注册失败（跳过）:', e instanceof Error ? e.message : String(e)) } catch { /* 忽略 */ }
  }
}
