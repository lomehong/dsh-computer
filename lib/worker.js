...
export class ComputerWorker {
    ...
    run(op, args = {}, timeoutMs = OP_TIMEOUT_MS) {
        const task = this.chain.catch(() => { }).then(() => this.runOnce(op, args, timeoutMs));
        this.chain = task;
        return task;
    }
    dispose() {
    ...
    async runOnce(op, args, timeoutMs) {
    ...
    async ensure() {
        ...
        this.touchIdle();
        const child = spawn('powershell.exe', ['-NoProfile', '-NonInteractive', '-ExecutionPolicy', 'Bypass', '-File', WORKER_PS1], { stdio: ['pipe', 'pipe', 'pipe'], windowsHide: true });
        this.child = child;
        child.on('exit', () => {
            this.child = undefined;
